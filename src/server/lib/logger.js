  ...
  transports: [
    new winston.transports.Console({
      format: combine(
        colorize(),
        timestamp(),
        printf(log => `${log.timestamp} ${chalk.blackBright(process.pid)} ${log.level} ${log.message}`)
      ),
...
module.exports = (namespace = '') => {
  ...
  return levels.reduce((acc, level) => {
    acc[level] = function(...args) {
      const verbosity = store.get('log.verbosity');
      if ((verbosity >= VERBOSITY_MAX) && (level !== 'silly')) {
...
module.exports.getLevel = () => logger.level;
